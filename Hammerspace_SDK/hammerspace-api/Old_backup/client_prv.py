...
class HammerspaceApiClient:
    ...
    def make_rest_call(
        self,
        path: str, # Can be a full URL if polling a Location header
        method: str = "GET",
        ...
        is_login: bool = False,
        is_absolute_url: bool = False # Flag to indicate if 'path' is a full URL
    ) -> requests.Response:
        """
        ...
        logger.debug(f"Request: {method} {url} Params: {query_params} JSON: {json_data is not None} Data: {data is not None} Files: {files is not None}")
        ...
        try:
            response = self.session.request(
                ...
                verify=self.verify_ssl, headers=headers, files=files, stream=stream
            )
    ...
    def execute_and_monitor_task(
        ...
        # Handle synchronous success (e.g., 200 OK, 201 Created with full object)
        ...
        # Handle asynchronous task initiation (202 Accepted)
        if initial_response.status_code == 202:
            ...
            if not location_url:
                ...
                # Return the body of the 202 if it exists, might contain a task object/ID
            ...
            last_known_status_data = None # To store the last successfully polled task data

            while (time.time() - start_time) < task_timeout_seconds:
                logger.debug(f"Polling task status at: {location_url}")
                try:
                    # The Location header provides the FULL URL for polling
                    task_status_response = self.make_rest_call(
                        path=location_url, # This is the full URL
                        method="GET",
                        is_absolute_url=True # Indicate that 'path' is a full URL
                    )
                ...
                if current_task_data and isinstance(current_task_data, dict):
                    ...
                    if not task_state: # If 'state' or 'status' not found, check 'statusMessage'
                    ...
                    if task_state == "COMPLETED":
                        ...
                        # Prefer 'result' if available, otherwise return the whole task object
                        ...
                        if isinstance(result_data, dict):
                            ...
                            if not entity_uuid and result_data.get('ctxMap') and isinstance(result_data['ctxMap'], dict):
                                ...
                                if isinstance(entity_uoid_val, dict) and entity_uoid_val.get('uuid'):
                                    ...
                                    except IndexError: pass # Keep entity_uuid as None
                            
                            if entity_uuid:
                                logger.info(f"Task completed, extracted entity UUID: {entity_uuid}")
                                # You might want to return just the UUID or the whole result_data
                                # For now, returning the result_data which might contain more info
                                return result_data 
                        return result_data # Return the result field or the whole task object
                        ...
                        return current_task_data # Return full task data on failure
                else:
        ...
        # If not 200, 201, or 202, it's likely an error or unexpected synchronous response
