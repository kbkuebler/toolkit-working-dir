...
# --- Example Usage ---
# Configuration (replace with your actual values for Hammerspace)
API_BASE_URL = "https://192.168.2.10:8443/mgmt/v1.2/rest" # e.g., "https://your-hs-instance:8443/mgmt/v1.2/rest"
USERNAME = "admin"
PASSWORD = "Password123!"
VERIFY_SSL_CERT = False # Typically False for lab/self-signed certs

if __name__ == "__main__":
    # Example: Creating a share (assuming this is an async operation)
    # This payload needs to be correct for your /shares POST endpoint
    share_creation_payload = {
        "comment": "My API Created Share",
        "name": "apiShareTest01",
        "path": "/apiShareTest01", # Or whatever path structure is expected
        "exportOptions": [{
            "subnet": "*",
            "accessPermissions": "RW", # Or "RO"
            "rootSquash": False,
            "insecure": False
        }]
        # Add other required fields for share creation from your OpenAPI spec
    }

    print("\n--- Attempting to create a share and monitor task ---")
    # Ensure the path is correct for creating shares, e.g., /shares
    # The OpenAPI spec you provided earlier didn't have a POST /shares,
    # so I'm assuming it exists. Adjust path and payload as needed.
    
    # Check your OpenAPI spec for the correct path to create a share.
    # For example, if it's /shares:

    # created_share_uuid = execute_and_monitor_task(
    #     base_url=API_BASE_URL,
    #     path="/shares", # Example path, verify from your API spec
    #     method="POST",
    #     initial_json_data=share_creation_payload,
    #     auth=(USERNAME, PASSWORD),
    #     verify_ssl=VERIFY_SSL_CERT,
    #     task_timeout_seconds=120 # Shorter timeout for testing
    # )

    DELETE_UUID = "e789bb91-03c8-47ff-bf80-00c5bf6b7e23"
    DELETE_PARAMS = {"delete-delay": "0", "delete-path": "true"}

    created_share_uuid = execute_and_monitor_task(
        base_url=API_BASE_URL,
        path=f"/shares/{DELETE_UUID}", # Example path, verify from your API spec
        method="DELETE",
        initial_query_params = DELETE_PARAMS,
        #initial_json_data=share_creation_payload,
        auth=(USERNAME, PASSWORD),
        verify_ssl=VERIFY_SSL_CERT,
        task_timeout_seconds=120 # Shorter timeout for testing
    ...
    if created_share_uuid:
        ic(f"Share Delete task completed. New Share UUID: {created_share_uuid}")
    else:
        ic("Share Delete task did not complete successfully or UUID not found.")
